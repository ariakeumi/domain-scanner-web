...
func Worker(id int, jobs <-chan string, results chan<- types.DomainResult, delay time.Duration, collector *stats.Collector) {
	WorkerWithContext(context.Background(), id, jobs, results, delay, collector)
}

// WorkerWithContext 域名检查工作协程，支持取消
func WorkerWithContext(ctx context.Context, id int, jobs <-chan string, results chan<- types.DomainResult, delay time.Duration, collector *stats.Collector) {
	for {
