...
- **Built-in Web UI**: Start a local browser console for configuring scans, tracking progress, canceling jobs, and exporting results
...
- `-web`: Start the browser-based web interface
- `-addr string`: Web server address for `-web` mode (default: 127.0.0.1:8080)
...
images (`linux/amd64`, `linux/arm64`) to Docker Hub whenever you push to the
default branch or a `v*` tag. It also runs as a build check on pull requests.

Before it can push, configure the following in
