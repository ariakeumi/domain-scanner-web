...
- **内置 Web 界面**：在本地浏览器中配置扫描、查看进度、取消任务并导出结果
...
- `-web`: 启动浏览器 Web 界面
- `-addr string`: `-web` 模式的监听地址（默认：127.0.0.1:8080）
- `-h`: 显示帮助信息
...
9. 启动 Web 界面：
```bash
go run main.go -web -addr 127.0.0.1:8080
```
...
自动构建多架构镜像（`linux/amd64`、`linux/arm64`）并推送到 Docker Hub。
拉取请求时也会作为构建检查运行。

在推送前，需要在 **GitHub → Settings → Secrets and variables → Actions** 中配置：

- **Secrets（密钥）**：`DOCKERHUB_USERNAME` 和 `DOCKERHUB_TOKEN`
  （在 https://hub.docker.com/settings/security 创建令牌）
- **Variable（变量，可选）**：`DOCKER_IMAGE` 用于覆盖仓库名称，
  例如 `yourname/domain-scanner`。默认为 `<owner>/<repo>`。

打 `v1.2.3` 标签会发布 `yourname/domain-scanner:1.2.3`、`:1.2` 和 `:latest`。
推送到默认分支会发布 `:latest` 以及 `sha-<短哈希>` 标签。
